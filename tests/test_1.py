...
def test_reconstruct_vector():
    ...
    try:
        auto_adpq = Auto_AdpQ(
            group_size=4, alpha=0.1, n_iters=10, q_bit=3, data_packing=True
        )
    ...
    auto_adpq_dp = Auto_AdpQ(
        group_size=8, alpha=0.1, n_iters=10, q_bit=4, data_packing=True
    )
    ...
    reconstructed_dp = auto_adpq_dp.reconstruct_vector(
        quantized_vectors_dp, outlier_indices_dp
    )
    expected_reconstructed_dp = np.zeros(2, dtype=np.int32)
    expected_reconstructed_dp[0] = (
        (1 << 0)
        | (2 << 4)
        | (3 << 8)
        | (4 << 12)
